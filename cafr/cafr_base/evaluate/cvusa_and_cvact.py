...
import gc
import copy
from ..trainer_area import predict
...
def evaluate(config,
   ...
   评估函数，用于评估模型在查询数据集上的性能
    ...
    r1 = calculate_scores(query_features, reference_features, query_labels, reference_labels, step_size=step_size,
                          ranks=ranks)
    if type(ra_dataloader) != int:
        query_labels_np = query_labels.cpu().numpy()

        query2index = dict()
        for i, idx in enumerate(query_labels_np):
            query2index[idx] = i
        ...
        near_dict = calculate_nearest(query_features=query_features,
                                      ...
                                      neighbour_range=config.rerank_n,
                                      step_size=step_size,
                                      save_true = 1)
        idx2label = []
        # idx2query = []
        for label,query_idx in query2index.items():
            idx2label = idx2label+near_dict[label]
            # idx2query.append(query_idx*config.rerank_n)
        ra_dataloader.dataset.idx2label = idx2label
        # ra_dataloader.dataset.idx2query = idx2query
        if config.rerank_n == 10:
            top_k = [1, 5,10]
        elif config.rerank_n == 5:
            top_k = [1,5]
        else:
            raise ValueError('ranks must be 5 or 10')
        results = np.zeros([len(top_k)])
        with torch.no_grad(), autocast():
            for i, (img, ids_current) in enumerate(tqdm(ra_dataloader)):
                # start = time.time()
                img = img.to(config.device)
                part_feat1,glob_feat1 = model(img)
                b,c,h, w = part_feat1.shape
                _, _, weights = model.apcm(query_features[i:i+1].expand(config.rerank_n,-1), part_feat1)
                attention_weight = weights.squeeze(1).view(b,h, w)
                attention_weight = attention_weight.unsqueeze(1)
                # attention_weight = torch.softmax(attention_weight, dim=(1, 2)).unsqueeze(1)
                part_feat2 = part_feat1 * attention_weight
                glob_feat2 = model.aggregator_atten(part_feat2)
                # print("extract:",time.time()-start)
                # start = time.time()
                results = calculate_scores_topn(query_features[i:i+1],glob_feat2,query_labels[i].cpu(),ids_current,top_k,results)
                # print("calculate:", time.time() - start)
        string = ['attention：']
        results = results / len(query_labels) * 100.
        for i in range(len(top_k)):
            string.append('Recall@{}: {:.4f}'.format(top_k[i], results[i]))
...
def calculate_scores_topn(query_features, reference_features, query_labels, reference_labels,
                     top_k,results):


    ref2index = dict()
    for i, idx in enumerate(reference_labels):
        ref2index[idx] = i

    similarity = query_features @ reference_features.T
    _,index = torch.sort(similarity,descending=True)
    sim_label = reference_labels[index.squeeze(0).cpu()].cpu()

    for i in range(len(results)):
        if query_labels in sim_label[:top_k[i]]:
            results[i] += 1.
    return results
...
def calculate_nearest(query_features, reference_features, query_labels, reference_labels, neighbour_range=64, step_size=1000,save_true = 0):
    """
    计算最近邻
    参数:
